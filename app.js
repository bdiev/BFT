...
function resetRestTimer() {
	...
	// Функция управления плавающей кнопкой CTA
	function initFloatingButton() {
		...
		function checkButtonPosition() {
			...
			// Получаем положение последнего элемента на странице
			const footer = document.querySelector('.site-footer');
			const appContent = document.getElementById('appContent');
			const contentToCheck = footer || appContent;
			...
			const contentRect = contentToCheck.getBoundingClientRect();
			const windowHeight = window.innerHeight;
			
			// Если контент виден на экране (top < window height)
			if (contentRect.top < windowHeight + 100) {
				// Переводим кнопку в anchored режим
				if (!btn.classList.contains('anchored')) {
					btn.classList.remove('floating');
					btn.classList.add('anchored');
				}
			} else {
				// Возвращаем плавающий режим
