...
(async () => {
	...
	const features = document.querySelectorAll('.carousel-feature');
	
	function initCarousel() {
		if (!carouselDots || features.length === 0) return;
	...
	function nextSlide() {
		carouselIndex = (carouselIndex + 1) % features.length;
		updateCarousel();
		resetAutoScroll();
	}
	
	function prevSlide() {
		carouselIndex = (carouselIndex - 1 + features.length) % features.length;
		updateCarousel();
		resetAutoScroll();
	}
	...
	function startAutoScroll() {
		...
		const timeToShow = parseInt(currentFeature?.dataset.time || 5000);
